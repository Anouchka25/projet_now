...
BEGIN
  ...
  IF direction LIKE 'GABON_TO_%' THEN
    -- Amount is in XAF, convert to EUR
    amount_in_eur := amount * 0.001524; -- Using exact XAF to EUR rate
    max_amount := 500; -- 500 EUR limit for transfers from Gabon
    max_amount_xaf := 327980; -- 500 EUR in XAF
    
    RETURN QUERY SELECT 
      amount_in_eur <= max_amount as valid,
      CASE 
        WHEN amount_in_eur <= max_amount THEN 'Montant valide'
        ELSE 'Le montant maximum autorisé pour les transferts depuis le Gabon est de ' || max_amount_xaf || ' XAF'
      END as message,
      max_amount_xaf;
  ELSE
